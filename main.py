...
def main():
    nodes_data = []
    print(f"开始抓取节点... 当前时间: {beijing_time}")
    ...
    for url in URL_SOURCES:
        try:
            ...
            if isinstance(content, dict):
                proxies_list = content.get('proxies') or content.get('outbounds') or ([content] if 'server' in content else [])
    ...
    if not nodes_data:
        print("❌ 严重错误: 未能捕获到任何有效节点！")
        return

    print(f"共捕获到 {len(nodes_data)} 个原始节点。")
    ...
    for info in nodes_data:
        ...
        if info["type"] == "tuic":
            ...
            ropts = info["item"].get('reality-opts') or info["tls_data"].get('reality') or {}
    ...
    for n in nodes_data:
        p = {
            "name": n["name"], "server": n["server"], "port": n["port"],
            "udp": True, "tls": True, "sni": n["sni"], "skip-cert-verify": True
        }
    ...
    print(f"✅ 执行完毕！已本地保存 node.txt({len(unique_links)}条), sub.txt, clash.yaml({len(clash_proxies)}条)")
